...
- **Core Responsibilities:**
  ...
  - **归档流程**（archive-order.md → 加载 updater-archive）：
    ...
    - 语义合并后追加 `memory/permanent-memory.md`（反 AI 规则）+ 写作记忆（`memory/writing-memory.md`）
  ...
  - **记忆兜底流程**（memory-sweep-order.md → 加载 memory-recording）：
    ...
    - 永久记忆晋升（use_count >= 4 的条目 → 移至 memory/permanent-memory.md）
...
- **输出产物（归档流程）:**
  ...
  - `memory/writing-memory.md` → 追加语义合并后的文风偏好（作家反馈）
...
- **Loop Integration:**
  ...
  LOAD SKILL:
    读 order 判断类型：
    ├── archive-order.md → 加载 novel-agents skill 的「updater-archive」SOP
    ├── setting-update-order.md → 加载 novel-agents skill 的「updater-setting」SOP
...
- **Allowed Tools:**
  | 工具 | 允许 | 禁止 |
  |------|------|------|
  | read | `settings/`、`archives/`、`chapters/`、`volumes/`（读卷纲中的 `## 设定变更通知` 块）、`memory/`、`.agent/` | 不读 prompts/ |
