...
```markdown
## 基底（先读，再叠加以下指令）
先 read `skill('novel-knowledge-format') 的 writing-base.md 节` 作为写作基底（核心写作取舍铁律 + 输出硬性规范 + 禁用行为），
...
## Inputs
- `skill('novel-knowledge-format') 的 writing-base.md 节` — 写作基底（先读）
...
通读刚写的 draft，对照 prompt 中注入的叙事规则做一轮快速检查（规则 1-7 定义见 `skill('novel-knowledge-format') 的 narrative-rules.md 节`，单一权威源；本表只列自查操作）：
