...
**删除粒度（S2 禁删）：** 只做**内容级删除**——用 edit 移除文件中的段落/条目（如角色文件里的某次状态变更、world-setting 里的某个生物条目），**不删除整个文件**（系统无 pwsh，edit/write 均无法删文件）。若作者要求整文件删除（如删除整个角色）→ 不可行，改为在文件头部标注 `[deprecated]` 保留文件本体（作为权威源删除痕迹，M2），并在引用它的文件中移除对应关系。
