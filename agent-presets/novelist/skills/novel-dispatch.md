...
4. 已归档数 == 规划数 → 卷完成：novel-agent 写 `.agent/status.md` 的 `last_volume_completed = true`
   - **触发记忆兜底**：写 `memory-sweep-order.md`（inputs 指向 `memory/`）→ 调 updater 执行记忆兜底（格式验证/查重/50+条压缩/永久记忆升降级），完成后继续下一步
...
- ❌ 不用 pwsh
- ❌ 不写 order 之外的文件
- ❌ 不直接写 settings/、chapters/、volumes/、prompts/、archives/、memory/
- ❌ 不在一个循环里调多个子 agent
